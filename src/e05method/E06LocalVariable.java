...
public class E06LocalVariable {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 간은 이름의 변수를 선언할 수 없다. 큰 지역에서 이미 선언되어
		 메모리에 상주하고 있기 때문이다.
		 */
		
		/* int num = 9;  아래코드에서 에러가 발생됨  */ 		
		...
		if(scope) {
			int num =1;
			num++;
			System.err.println("첫번째 if문 지역:" + num);
		}
		else {
			int num = 5;
	...
	/*main에서 선언한 변수 num은 아직 메모리에 상주하고있지만
	 해당 메서드는 크기가 동일한 서로 다른 지역에 정의 되었으므로
	 같은 이름의 변수를 선언할 수 있다. 앞의 
	 main 메서드의 if~else문의 블럭도 같은 개념으로 변수를 선언한
	 것이다.*/
