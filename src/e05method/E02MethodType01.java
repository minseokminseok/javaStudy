...
-상수명 : SIMPLE_FUNC
=>전체를 대문자로 기술한다. 단 연결되는 단어는_로 구분한다.
-패키지명 : simple.func
...
/*
 매서드형태1] 매개변수도 없고, 반환값도 없는 매서드
 	: 둘 다 없는 형태의 매서드로 주로 메뉴를 출력하거나 하는
 	단순 출력 기능 정도로 사용한다.
 */
public class E02MethodType01 {
	...
	static void returnError() {
		...
		/*이와 같이 매서드 중간에 return 을 기술하면 즉시 종료되므로
		 그 아래에 있는 코드는 실행되지 않는 쓰레기 코드가 된다.
		 따라서 return 을 사용할때는 반드시 조건문과 같이 기술되어야한다.
		 단 매서드의 마지막 라인에 기술한다면 조건문이 없어도 된다.*/
		//return;
