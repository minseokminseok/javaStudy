...
/*
시나리오] 여러개의 숫자를 입력받은후 최대값 구하기
단 숫자의 갯수는 매개변수로 전달받고 
숫자의 갯수만큼 Scannaer 클래스를 통해 입력받는다.
	...
	static int returnMaxNumber(int numberCnt) {
		...
		for(int i=1; i<=numberCnt; i++) {
			...
			if(i==1) {
				...
				//첫번째 입력값을
				// 비교를 위한 기준값으로 설정한다. 
				//비교의 대상이 없으므로 무조건 최대값
				maxVal = inputNum;
			}
			else {
				/*두번째 입력값 부터는 기존의 최대값과 비교해서
				 더 큰 값이 발견되면 교체한다.*/
				if(maxVal<inputNum) {
					maxVal = inputNum;
				}
		...
		//즉, 새로운 입력값이 기존값보다 크다면 
		//지속적으로 교체하여 가장 큰값을 호출한 지점으로 반환
		return maxVal;
	}
