...
/*
 ...
 메서드의 지역이 다르더라도 참조값을 통해 객체를
 참조하게 되므로 
 A지역에서의 값의 변경이 B지역에서도 그대로 참조할수 있는
 장점이있다. 참조값만 전달하므로 프로그램의 퍼포먼스(성능)도 향상되고
 메모리도 절약할수 있다.
 */
public class E04CallByReference {
	...
	static void callByReference(int[] ref) {
		/*매개변수를 통해 전달받은 배열객체의 참조값을 통해 
		 0번 원소와 1번원소를 서로 교환한다. 이때 배열의 값이 서로 
		 바뀌게 된다.*/
		int temp;
		temp = ref[0];
		ref[0] = ref[1];
